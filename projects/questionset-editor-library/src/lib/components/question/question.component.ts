      ...
      if (_.isUndefined(this.questionId)) {
        ...
        if (this.questionInteractionType === 'default') {
          ...
          if (this.questionInput?.config?.isTrueFalseQuestion) {
            ...
            this.editorState = new McqForm({ question: defaultState.question, options: _.shuffle(defaultState.options)}, { numberOfOptions: optionsCount, maximumOptions: optionsCount });
          }
        }
        else if (_.includes(['match', 'order'], this.questionInteractionType)) {
          const FormClass = this.getFormClass();
